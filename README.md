...
<img src="https://github.com/avenom/OBS2HA/blob/main/Images/obs2ha.jpg">
...
В гайде возможны некоторые неточности. Не хотелось все удалять и настраивать заново для проверки гайда. Если что не работает пишите в [телеграм](https://t.me/avenom).
...
Убедитесь, что все названия сцен в OBS Studio написаны на английском языке без пробелов и дефисов. Это нужно, чтобы скрипт OBS2HA.py корректно обрабатывал имена сцен и передавал их в Home Assistant.
...
4. Скачайте скрипт **[OBS2HA.py](https://github.com/avenom/OBS2HA/blob/main/OBS2HA.py)** в удобное вам место и откройте файл в текстовом редакторе. Найдите и измените в нем следующие строчки с настройками подключения:
```python
...
2. Откройте веб-интерфейс Home Assistant. Перейдите в **«Настройки» → «Устройства и службы»** и выберите **MQTT**. Если все работает нормально, то должна отображаться работа службы OBSHA. Открыв ее вы должны будете увидеть все ваши сцены в OBS Studio с именем типа Scene Game. Можете проверить работу переключения сцен с помощью переключателей.
---
